...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $readonly_fields = ['username', 'user_name']; // Fields that shouldn't be editable
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                echo "<script>
                    ...
                    window.location.href = 'adminCheckUserInfo.php';
                </script>";
                exit();
            } else {
                echo "<script>alert('Error updating user profile: " . $stmt->error . "');</script>";
            }
            $stmt->close();
        } else {
                ...
                <div class="flex justify-center mt-12">
                    <div class="w-full max-w-2xl bg-white border border-gray-200 rounded-lg shadow-lg">
                        <div class="p-6 border-b border-gray-200">
                            ...
                            <p class="text-sm text-gray-600">Update user profile details below.</p>
                            <div class="mt-2">
                                <a href="adminCheckUserInfo.php" class="text-blue-500 hover:text-blue-700 text-sm">← Back to User List</a>
                            </div>
                        </div>
                        ...
                        <form method="POST" class="p-6 space-y-4">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <?php
                                // Dynamic form generation based on user table fields
                                $skip_fields = ['id', 'password', 'created_at', 'updated_at'];
                                $readonly_fields = ['username', 'user_name']; // Fields that shouldn't be editable
                                ...
                                foreach ($user as $field_name => $field_value) {
                                    ...
                                    if (strpos($field_name, 'email') !== false) {
                                        ...
                                        $input_type = 'date';
                                    } elseif (strpos($field_name, 'birth') !== false) {
                                        $input_type = 'date';
                                    } elseif (strpos($field_name, 'age') !== false) {
                                        $input_type = 'number';
                                    }
                                ?>
                                    <div class="<?php echo (strpos($field_name, 'address') !== false || strpos($field_name, 'bio') !== false || strpos($field_name, 'note') !== false) ? 'md:col-span-2' : ''; ?>">
                                        ...
                                        <?php if (strpos($field_name, 'address') !== false || strpos($field_name, 'bio') !== false || strpos($field_name, 'description') !== false || strpos($field_name, 'note') !== false): ?>
                                            <textarea 
                                        ...
                                        <?php else: ?>
                                            <input 
                                                ...
                                                <?php echo $readonly_attr; ?>
                                                <?php if ($input_type === 'number'): ?>min="0" max="120"<?php endif; ?>
                                                class="mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400 <?php echo $readonly_class; ?>"
                                            >
                                        <?php endif; ?>
                                    </div>
                            ...
                            <div class="pt-6 border-t border-gray-200">
                                <div class="flex space-x-4">
                                    <button type="submit"
                                        class="flex-1 bg-green-500 text-white font-medium py-2 px-4 rounded-lg shadow-md hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-400 focus:ring-offset-2 transition-colors">
                                        Save Changes
                                    </button>
                                    <button type="button" 
                                        onclick="window.location.href='adminCheckUserInfo.php'"
                                    ...
                                    <button type="button" 
                                        ...
                                        class="px-6 bg-blue-500 text-white font-medium py-2 rounded-lg shadow-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 transition-colors">
                                        Reset
                                    </button>
                                </div>
                            </div>
                        ...
                        <div class="p-6 border-t border-gray-200 bg-gray-50">
                            <h3 class="text-lg font-semibold text-gray-800 mb-4">Current User Information</h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                <?php foreach ($user as $field_name => $field_value): ?>
                                    <div>
                        ...
                        $stats_query = "SELECT 
                            ...
                            (SELECT MIN(created_at) FROM bookings WHERE user_id = ?) as first_booking,
                            (SELECT MAX(created_at) FROM bookings WHERE user_id = ?) as last_booking";
                        ...
                        <div class="p-6 border-t border-gray-200 bg-blue-50">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                                <div class="bg-white p-3 rounded-lg">
                                    <span class="font-medium text-blue-600">Total Bookings:</span>
                                    <div class="text-xl font-bold text-gray-800"><?php echo $stats['total_bookings'] ?? 0; ?></div>
                                </div>
                                <div class="bg-white p-3 rounded-lg">
                                    <span class="font-medium text-green-600">First Booking:</span>
                                    <div class="text-sm text-gray-800"><?php echo $stats['first_booking'] ? date('M d, Y', strtotime($stats['first_booking'])) : 'No bookings'; ?></div>
                                </div>
                                <div class="bg-white p-3 rounded-lg">
                                    <span class="font-medium text-purple-600">Last Activity:</span>
                                    <div class="text-sm text-gray-800"><?php echo $stats['last_booking'] ? date('M d, Y', strtotime($stats['last_booking'])) : 'No recent activity'; ?></div>
                                </div>
                            </div>
                        </div>
                    </div>
